...
class browser(object):
    ...
    def createBrowser(self,PROXY):
        ...
        '''webdriver.DesiredCapabilities.FIREFOX['proxy'] = {
            ...
            #"sslProxy":PROXY,
            #"socksProxy": PROXY[:a],
            #"socksProxyPort": (PROXY[(a+1):]),
            #"socksVersion": 4
        }'''

        fp = webdriver.FirefoxProfile()
        fp.set_preference("network.proxy.type", 1)
        fp.set_preference("network.proxy.socks", str(PROXY[:a]))
        fp.set_preference("network.proxy.socks_port", int(PROXY[(a+1):]))
        ...
        driver = webdriver.Remote(
            browser_profile=fp, command_executor='http://172.18.0.4:5555/wd/hub')

        driver.set_page_load_timeout(30)
        return driver

    def open_browser(self, url, repeat):
        while(repeat>0):
            try:
                proxy = self.proxies()
                br = self.createBrowser(proxy)
                print("selected proxy: " + str(proxy))
                sleep(3)
                br.get("https://myexternalip.com/")
                sleep(5)
                br.get(url)
                sleep(5)
                if str(br.title) == url:
                    raise Exception
                print(str(repeat) + " -- "+ str(br.title))
                sleep(75) #view second
                repeat=repeat-1
                br.quit()
                return self.open_browser(url,repeat)
            except:
                br.quit()
    ...
    def find_proxy(self):
        p = subprocess.Popen(['proxybroker', '--timeout', '1', 'find', '--types', 'SOCKS5', '--strict',
                              '-l 0', '--format', 'json', '--outfile', 'proxies.json'])
        try:
            p.wait(60)
        ...
        with open('proxies.json') as json_file:
            data = json.load(json_file)
            index = 0
            proxy_file = open("proxies", "w")
            while index < len(data):
                proxy = data[index]['host'] + ':' + str(data[index]['port'])
    ...
    def proxies(self):
        ...
        if p_index < len(proxylist):
            return proxylist[p_index]
        elif p_index == len(proxylist):
            p_index = -1
            return self.proxies()

def main():
    if len(sys.argv) != 3:
        print("Please run script with right parameters.\nbot.py [video-url] [view-number]")
    else:
        url = sys.argv[1]
        repeat = sys.argv[2]
        br = browser()
        ...
        with open("proxies") as f:
            proxylist = [line.rstrip() for line in f]
        br.open_browser(url,int(repeat))
        with open("proxies2", "w") as f:
